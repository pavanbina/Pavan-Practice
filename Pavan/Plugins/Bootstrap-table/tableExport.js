...
*/
/// <reference path="../JSPDFs/html2pdf.js" />
/// <reference path="../JSPDFs/jspdf.debug.js" />
        ...
        tableExport: function (options) {
            var defaults = {
                ...
                jspdf: { orientation: 'l',
                    ...
                    margins: { left: 30, right: 30, top: 40, bottom: 30 },
            ...
            if (defaults.type == 'csv' || defaults.type == 'txt') {
                ...
                var $tables = $(el).filter(function () {
                   ...
                   ($(this).is(':visible') ||
                    $(this).data("tableexport-display") == 'always' || $(this).is(':isexportval'));
                });
                var docData = '';
              ...
              function (cell, row, col) {
                  if (cell != null && cell != "") {
                      ...
                      //commented above line and add below code
                      // changed by sujatha G for remove selected headers   
                      //start
//                      if ($(cell).is("[colspan]"))
//                          trData += '" colspan="' + $(cell).attr('colspan');
//                      if ($(cell).is("[rowspan]"))
...
//                      var tblcolumnname = JSON.stringify(parseString(cell, row, col));
//                      if (tblcolumnname != "Operate" || tblcolumnname != "") {
//                          trData += '">' + parseString(cell, row, col) + '</th>';
//                          console.log('cell val in fun ' + JSON.stringify(parseString(cell, row, col)));
//                      }
                   
                      // end region by sujatha
                  }
              });
                        if (trData.length > 0)
                            docData += '<tr>' + trData.replace("Item Operate", " ") + '</tr>'; //changed by sujatha-- add replce method   trData.replace("Item Operate", " ") 
              ...
              function (cell, row, col) {
                  if (cell != null) {
                    ...
                    //Nagarjuna only Do this Things strat
                    $rows = $('.fixed-table-footer').find('tbody').first().find(defaults.tbodySelector);
                    $rows.each(function () {
                        trData = "";
                        ForEachVisibleCell(this, 'td', rowIndex, $hrows.length + $rows.length,
              function (cell, row, col) {
                ...
                //Nagarjuna only Do this Things End
                ...
                docFile += "</head>";
                docFile += "<body>";
                docFile += docData; //.replace("Item Operate", " "); changed by sujatha
                docFile += "</body>";
                docFile += "</html>";
                   ...
                   ($(this).is(':visible') ||
                    $(this).data("tableexport-display") == 'always');
                      ...
                      function (cell, row, col) {
                        ...
                        //Nagarjuna only Do this Things strat
                        $rows = $('.fixed-table-footer').find('tbody').find(defaults.tbodySelector);
                        $rows.each(function () {
                            var rowData = [];
                            colKey = 0;
